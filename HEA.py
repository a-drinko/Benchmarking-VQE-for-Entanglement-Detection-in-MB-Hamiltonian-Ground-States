# 1) create folders "HEA"->"Temporary_Files"
...
arquivo = open('HEA/HEA '+str(qbits)+' qbits '+str(Nshots)+' Shots.txt', 'w')
...
def F( x , y ):
    Arq = open('HEA/Temporary_Files/file'+str(x)+'.txt', 'w')
    for j in range(y):
...
def VQA(random):
    ...
    Arquivo = open('HEA/Temporary_Files/file'+str(random)+'.txt', 'w')
    opt = qml.GradientDescentOptimizer(stepsize=0.1)
    
    rotations = np.array([[np.random.uniform(low=-np.pi, high=np.pi) for i in range(qbits)] for i in range(nLayers)],
                                 requires_grad=True)
    params = rotations
...
for x in range(len(random)):
    r1 = np.loadtxt('HEA/Temporary_Files/file'+str(x)+'.txt')
    Results.append(r1)
...
DadosMedia = open('HEA/HEA '+str(qbits)+' qbits '+str(Nshots)+' Shots Mean.txt', 'w')
for i in range(steps):
...
ax12.set_title('HEA '+str(qbits)+' qbits, '+str(Nshots)+' shots')
...
plt.legend()
plt.savefig('HEA/HEA '+str(qbits)+' qbits '+str(Nshots)+' shots.pdf', format='pdf')
