# 1) create folders "Syc Ispired"->"Temporary_Files"
...
#Syc Inspired ansatz
def Layer(x):
    for w in range(qbits-1):
        for i in range(w,w+2,1):
            qml.RX(x[i+w], wires=i)
        for i in range(w,w+1,1):
            qml.IsingXY(-4*x[i+qbits+w],wires=[i,i+1])
            qml.IsingZZ(-2*x[i+qbits+1+w],wires=[i,i+1])
        for i in range(w,w+2,1):
            qml.RZ(x[i+2*qbits+w], wires=i)
...
arquivo = open('Syc Inspired/Syc Inspired '+str(qbits)+' qbits '+str(Nshots)+' Shots.txt', 'w')
...
def F( x , y ):
    Arq = open('Syc Inspired/Temporary_Files/file'+str(x)+'.txt', 'w')
    for j in range(y):
...
def VQA(random):
    ...
    Arquivo = open('Syc Ibspired/Temporary_Files/file'+str(random)+'.txt', 'w')
    opt = qml.GradientDescentOptimizer(stepsize=0.01)
    
    rotations = np.array([[np.random.uniform(low=-np.pi, high=np.pi) for i in range((qbits-1)*6)] for i in range(nLayers)],
                                 requires_grad=True)
    params = rotations
...
for x in range(len(random)):
    r1 = np.loadtxt('Syc Inspired/Temporary_Files/file'+str(x)+'.txt')
    Results.append(r1)
...
DadosMedia = open('Syc Inspired/Syc Ispired '+str(qbits)+' qbits '+str(Nshots)+' Shots Média.txt', 'w')
for i in range(steps):
...
ax12.set_title('Syc Inspired '+str(qbits)+' qbits, '+str(Nshots)+' shots')
...
plt.legend()
plt.savefig('Suc Inspired/Syc Inspired '+str(qbits)+' qbits '+str(Nshots)+' shots.pdf', format='pdf')
