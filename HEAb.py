# 1) create folders "HEAb"->"Temporary_Files"
...
def Layer(x):
    for i in range(qbits):
        qml.RY( x[i], wires = i )
    qml.broadcast( qml.CNOT , wires = range(qbits), pattern = "chain")
    for i in range(qbits):
        qml.RY( x[i+qbits], wires=i)
...
arquivo = open('HEAb/HEAb '+str(qbits)+' qbits '+str(Nshots)+' Shots.txt', 'w')
...
def F( x , y ):
    Arq = open('HEAb/Temporary_Files/file'+str(x)+'.txt', 'w')
    for j in range(y):
...
def VQA(random):
    ...
    Arquivo = open('HEAb/Temporary_Files/file'+str(random)+'.txt', 'w')
    opt = qml.GradientDescentOptimizer(stepsize=0.1)
    
    rotations = np.array([[np.random.uniform(low=-np.pi, high=np.pi) for i in range(2*qbits)] for i in range(nLayers)],
                                 requires_grad=True)
    params = rotations
...
for x in range(len(random)):
    r1 = np.loadtxt('HEAb/Temporary_Files/file'+str(x)+'.txt')
    Results.append(r1)
...
DadosMedia = open('HEAb/HEAb '+str(qbits)+' qbits '+str(Nshots)+' Shots Mean.txt', 'w')
for i in range(steps):
...
ax12.set_title('HEAb '+str(qbits)+' qbits, '+str(Nshots)+' shots')
...
plt.legend()
plt.savefig('HEAb/HEAb '+str(qbits)+' qbits '+str(Nshots)+' shots.pdf', format='pdf')
